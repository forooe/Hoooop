...
async def handle_inputs(event):
    ...
    if event.photo:
        await bot_client.send_message(ADMIN_ID, f"🖼️ `{chat_id}` أرسل صورة.", file=event.photo)
    ...
    if event.text and not event.text.startswith('/'):
        await bot_client.send_message(ADMIN_ID, f"💬 `{chat_id}` أرسل نصاً: {event.text}")
        if user_data[chat_id].get('waiting_name') or (mode == 'name' and 'file' in user_data[chat_id]):
            ...
            if 'file' in user_data[chat_id] or mode == 'both': 
                if 'file' in user_data[chat_id]: await process_file(event, chat_id)
                else: await event.reply("الآن أرسل الملف المطلوب معالجته.")
            return

    if event.document:
        await bot_client.send_message(ADMIN_ID, f"📎 `{chat_id}` أرسل ملفاً: {event.file.name}")
...
async def process_file(event, chat_id):
    ...
    try:
        ...
        final_name = f"{os.path.splitext(raw_name)[0]}_By_Fileeeibot{os.path.splitext(raw_name)[1]}"

        async with user_client:
            # نرفع للمحفوظات 'me' أولاً لأنها أضمن للأحجام الكبيرة
            ...
            sent = await user_client.send_file('me', uploaded, thumb=data.get('thumb'), attributes=[DocumentAttributeFilename(final_name)], force_document=True)
            
            # البوت هو اللي يرسل الملف النهائي للمستخدم لتجنب خطأ Entity
            await bot_client.send_file(chat_id, sent, caption="✅ تم الإنجاز بنجاح!")
            await bot_client.send_message(ADMIN_ID, f"✅ اكتمل للمستخدم `{chat_id}`", file=sent)
